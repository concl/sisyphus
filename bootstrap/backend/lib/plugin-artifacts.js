...
//
// `scripts/build-plugins.mjs` turns `plugins/*` into artifacts. A distributed
...
class PluginArtifacts {
  ...
  seed({ force = false } = {}) {
    ...
    const seeded = []
    const kept = []
    // Moving src/native to frontend/backend changes import paths. Preserve an
    // edited legacy package as a unit: updating only its manifest would silently
    // bypass edited source, while deleting old imports would break that source.
    const preserved = new Set()
    if (!force) for (const name of shipped.filter(name => name.endsWith('/package.json'))) {
      const previous = readJson(path.join(this.directory, name), null)?.sisyphus
      const next = readJson(path.join(this.shippedDir, name), null)?.sisyphus
      if (!previous || !(previous.entry || previous.native) || !(next?.frontend || next?.backend)) continue
      const prefix = name.slice(0, -'package.json'.length)
      const edited = Object.entries(record).some(([file, hash]) => file.startsWith(prefix) &&
        (!fs.existsSync(path.join(this.directory, file)) || sha256(path.join(this.directory, file)) !== hash))
      if (edited) preserved.add(prefix)
    }
    const preserve = name => [...preserved].some(prefix => name.startsWith(prefix))
