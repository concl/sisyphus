...
// OS probes can take seconds (especially WSL). Never run them on Electron's loop.
const exec = require('node:util').promisify(require('node:child_process').execFile)
async function detectBackendsAsync() {
  if (!WINDOWS) return detectBackends()
  const backends = [{ id: 'powershell', name: 'PowerShell', command: 'powershell.exe', args: ['-NoLogo'] }]
  const [pwsh, wsl] = await Promise.all([
    exec('where.exe', ['pwsh.exe'], { windowsHide: true, timeout: 4000 }).then(() => true, () => false),
    exec('wsl.exe', ['-l', '-q'], { windowsHide: true, timeout: 4000, encoding: 'buffer' })
      .then(({ stdout }) => stdout.toString(stdout.includes(0) ? 'utf16le' : 'utf8').replace(/^\uFEFF/, '').split(/\r?\n/).find(line => line.trim())?.trim(), () => null),
  ])
  if (pwsh) backends.push({ id: 'pwsh', name: 'PowerShell 7', command: 'pwsh.exe', args: ['-NoLogo'] })
  backends.push({ id: 'cmd', name: 'Command Prompt', command: process.env.COMSPEC || 'cmd.exe', args: [] })
  const bash = findGitBash()
  if (bash) backends.push({ id: 'gitbash', name: 'Git Bash', command: bash, args: ['--login', '-i'] })
  if (wsl) backends.push({ id: 'wsl', name: `WSL: ${wsl}`, command: 'wsl.exe', args: ['-d', wsl] })
  return { backends, defaultId: 'powershell' }
}
module.exports.detectBackendsAsync = detectBackendsAsync
