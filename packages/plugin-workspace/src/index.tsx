    ...
    apply(ctx) {
      ...
      function Workspace() {
        ...
        async function ready({ api }: DockviewReadyEvent) {
          ...
          const removeMissing = () => {
            const ids = new Set(panels.list().map((panel) => panel.id))
            for (const panel of [...api.panels])
              if (!ids.has(panel.params?.type)) api.removePanel(panel)
          }
          ...
          try {
            ...
            if (saved) {
              ...
              api.fromJSON({ ...saved, floatingGroups: [], popoutGroups: [] })
              removeMissing()
            } else reset()
          } catch {
            if (alive) {
        ...
        function openNamedLayout(name: string) {
          try {
            const api = dock.current
            if (!api) return
            api.fromJSON({ ...savedLayouts[name], floatingGroups: [], popoutGroups: [] })
            const ids = new Set(panels.list().map((panel) => panel.id))
            for (const panel of [...api.panels])
              if (!ids.has(panel.params?.type)) api.removePanel(panel)
        ...
        async function toggle(plugin: PluginStatus, native = false) {
          ...
          try {
            ...
            else await runtime.setEnabled(plugin.id, !plugin.enabled)
          } catch (error) {
            setMessage(String(error))
          } finally {
            setBusy(false)
          ...
          <div className="workspace">
            <header className="workspace-header">
              ...
              <div className="header-actions">
                ...
                <button
                  className={showPlugins ? 'selected' : ''}
                ...
                <div className="layout-control">
                  <button
                    ...
                    className={showLayouts ? 'selected' : ''}
                ...
                {available.some((panel) => panel.id === 'settings') && (
                  <button aria-label="Open Settings" onClick={() => panels.open('settings')}>
                    Settings
                  </button>
                )}
                <div className="add-block">
                  <button className="primary" onClick={() => setAddMenu(!addMenu)}>
                    ＋ Add block
                  </button>
                  {addMenu && (
              ...
              {showPlugins && (
                <aside className="plugin-drawer">
                  ...
                  <p className="drawer-note">
                    ...
                    its processes. Reopen blocks after enabling a provider.
                  </p>
