const { z } = require('zod')
const { FileBridge } = require('./lib/mcp-files.js')
const { FileIndex } = require('./lib/file-index.js')
const { resolveDroppedPaths } = require('./lib/dropped-files.js')
...
module.exports = ({ index } = {}) => ({
  ...
  apply(ctx) {
    ...
    // Native drag paths are resolved in the preload, then canonicalized and
    // checked against the same folder boundary as the agent's file tools.
    ctx.effect(() =>
      transport.handle('files.resolveDrop', ({ paths, folder }) =>
        resolveDroppedPaths(paths, folder),
      ),
    )
