...
export const themePlugin: AppPlugin = {
  ...
  apply(ctx) {
    ...
    const systemChanged = () => {
      if (state.theme === 'system') apply(resolve('system'))
    }
    media.addEventListener('change', systemChanged)
    const off = desktop.on<ThemeState & { platform: string }>('appearance.changed', apply)
    void desktop
      .call<ThemeState & { platform: string }>('appearance.get')
      .then(apply)
      .catch(() => {})
    const theme: Theme = {
      getSnapshot: () => state,
      ...
      async set(preference) {
        if (window.sisyphus)
          apply(await desktop.call<ThemeState>('appearance.set', { theme: preference }))
