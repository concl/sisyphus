...
const excluded = new Set(['node_modules', '.git', '.runtime', 'dist'])
...
function packages(directory) {
  ...
  for (const child of fs.readdirSync(directory, { withFileTypes: true })) {
    ...
    try {
      ...
      const entries = [...(meta.entry ? [{ ...meta, target: 'renderer' }] : []),
        ...(meta.native ?? []).map(entry => ({ ...entry, target: 'main' }))]
      for (const entry of entries) {
        const stamp = createHash('sha256').update(files.filter(file => {
          ...
          return entry.target === 'renderer' ? !relative.startsWith('native/')
            : !entry.entry.includes('/native/') || !relative.startsWith('src/')
        }).map(file => {
...
module.exports = { packages, compile, evaluateNative, filesIn, inside, createWorker }
