...
module.exports = () => ({
  ...
  apply(ctx) {
    ...
    let backends
    const getBackends = () => (backends ??= detectBackends())
    ...
    handle('backends', () => {
      const { backends, defaultId } = getBackends()
      return { backends: backends.map(({ id, name }) => ({ id, name })), defaultId }
    })
    handle('spawn', ({ id, backendId, cols, rows }, sender) => {
      ...
      const backend = getBackends().backends.find((item) => item.id === backendId)
